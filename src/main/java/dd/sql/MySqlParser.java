...
import io.trino.sql.parser.AstBuilder;
import io.trino.sql.parser.CaseInsensitiveStream;
import io.trino.sql.parser.ErrorHandler;
import io.trino.sql.parser.ParsingException;
import io.trino.sql.parser.ParsingOptions;
import io.trino.sql.parser.SqlBaseBaseListener;
...
public class MySqlParser {
    ...
    public SqlInfo parseInfo(String sql) {
        try {
            ...
            InfoBuilder builder = new InfoBuilder(parsingOptions);
            Node node = builder.visit(tree);
            SqlInfo info = builder.getInfo();
            info.setAstTree(node);
            return info;
        } catch (StackOverflowError e) {
            throw new ParsingException("sql is too large (stack overflow while parsing)");
        }
    }
